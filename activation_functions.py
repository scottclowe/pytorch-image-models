...
def get_pk_ratio(actfun, p, k, g):
    if actfun == 'groupsort':
        pk_ratio = p
    elif actfun == 'bin_part_full' or actfun == 'ail_part_full' or actfun == 'nail_part_full':
        pk_ratio = (p * (2 + k)) / (3 * k)
    elif actfun == 'bin_all_full' or actfun == 'ail_all_full' or actfun == 'nail_all_full':
        pk_ratio = p * ((3 / k) + 1)
    elif actfun == 'bin_all_max_min_sgm' or actfun == 'ail_all_or_and_xnor' or actfun == 'nail_all_or_and_xnor':
        pk_ratio = p * ((3 / k))
    elif actfun == 'bin_all_max_min' or actfun == 'ail_all_or_and' or actfun == 'nail_all_or_and':
        pk_ratio = p * ((2 / k))
    elif actfun == 'bin_all_max_sgm' or actfun == 'ail_all_or_xnor' or actfun == 'nail_all_or_xnor':
        pk_ratio = p * ((2 / k))
    else:
        pk_ratio = (p / k)
    return pk_ratio
...
def activate(x, actfun, p=1, k=2, M=None,
             layer_type='conv',
    ...
    for i in range(1, p):
        ...
        if permute_type == 'invert':
            if i % k == 0:
                ...
                curr_shuffle[0] = i % k
                curr_shuffle[i % k] = 0
        permutation = permute(x[:, :, permute_base, ...], curr_permute, layer_type, k,
...
_ACTFUNS = {
    ...
    'lse-approx':
        lambda z: torch.max(z[:, :, 0], z[:, :, 1]) + torch.max(torch.tensor(0., device=z.device),
                                                                _ln2 - 0.305 * (z[:, :, 0] - z[:, :, 1]).abs_()),
    'lae-approx':
        lambda z: torch.max(z[:, :, 0], z[:, :, 1]) + torch.max(torch.tensor(-_ln2, device=z.device),
                                                                -0.305 * (z[:, :, 0] - z[:, :, 1]).abs_()),
    'nlsen-approx':
        lambda z: -torch.max(-z[:, :, 0], -z[:, :, 1]) - torch.max(torch.tensor(0., device=z.device),
                                                                   _ln2 - 0.305 * (z[:, :, 0] - z[:, :, 1]).abs_()),
