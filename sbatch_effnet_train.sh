...
#SBATCH --output=logs_new/ef_tl_89_full/%a-%N-%j    # %N for node name, %j for jobID
#SBATCH --job-name=ef_tl_89_full
...
python train.py /scratch/ssd001/datasets/imagenet/ --seed $SEED --model tf_efficientnet_b0 --pretrained --tl --output $SAVE_PATH --resume $CHECK_PATH --epochs 2 --amp --lr $LR --batch-size 256
