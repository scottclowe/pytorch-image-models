...
#SBATCH --output=logs_new/nef_test/%a-%N-%j    # %N for node name, %j for jobID
#SBATCH --job-name=nef_test
...
~/utilities/log_gpu_cpu_stats -l 0.5 -n 1 -t "${SAVE_PATH}/${SLURM_ARRAY_TASK_ID}_${SLURM_NODEID}_${SLURM_ARRAY_JOB_ID}_compute_usage.log"&
export LOGGER_PID="$!"
