...
#SBATCH --output=logs_new/test_onecycle/%a-%N-%j    # %N for node name, %j for jobID
#SBATCH --job-name=test_onecycle
...
python train.py /scratch/ssd001/datasets/imagenet/ --model tf_efficientnet_b0 --pretrained --tl --output $SAVE_PATH --resume $CHECK_PATH --epochs 2 --amp --lr $LR
