...
#SBATCH --output=logs_new/ef_tl_ho_8full_9full/%a-%N-%j    # %N for node name, %j for jobID
#SBATCH --job-name=ef_tl_ho_8full_9full
...
python train.py /scratch/ssd001/datasets/imagenet/ --seed $SEED --model tf_efficientnet_b0 --pretrained --tl --output $SAVE_PATH --resume $CHECK_PATH --epochs 2 --amp --lr 1e-3 --batch-size 256 --tl-layers 8full_9full --actfun $ACTFUN --p 2 --k 2